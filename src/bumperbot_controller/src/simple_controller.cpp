...
using std::placeholders::_1;
  ...
  , theta_(0.0)
 {
   ...
   wheel_separation_ = get_parameter("wheel_seperation").as_double();

   RCLCPP_INFO_STREAM(this->get_logger(), "Using wheel radius: " << wheel_radius_);
   RCLCPP_INFO_STREAM(this->get_logger(), "Using wheel seperation: " << wheel_separation_);
 ...
 void SimpleController::jointCallback(const sensor_msgs::msg::JointState &msg)
 {
   ...
   // Calculate the position increment
    ...
    theta_ += d_theta;//update to the new oriention

    //update to the new position
    x_ += d_s * cos(theta_);
    y_ += d_s * sin(theta_);

   RCLCPP_INFO_STREAM(this->get_logger(),"Linear: "<<linear<<"Angular: "<<angular);
   RCLCPP_INFO_STREAM(this->get_logger(),"x: "<<x_<<"y: "<<y_<<"theta: "<<theta_);

...
void SimpleTurtlesimKinematics::turtle2PoseCallback(const turtlesim::msg::Pose &pose)
{
   ...
   // the distance between both the Translation vector
   float Tx = last_turtle1_pose_.x - last_turtle2_pose_.x;
   float Ty = last_turtle1_pose_.y - last_turtle2_pose_.y;

   float theta_rad = last_turtle1_pose_.theta - last_turtle2_pose_.theta;
...
int main(int argc, char const *argv[])
{
   ...
   auto node = std::make_shared<SimpleTurtlesimKinematics> ("simple_turtlsim_kinematics");
   rclcpp::spin(node);
   rclcpp::shutdown();
   return 0;
}

...
private: 
    ...
    // Timer and service to get the requested transforms
    rclcpp::TimerBase::SharedPtr timer_;
    rclcpp::Service<bumperbot_msgs::srv::GetTransform>::SharedPtr get_transform_srv_;

    // Publishes static and dynamic transforms
    ...
    // Used to look uoo transforms between frames 
    std::shared_ptr<tf2_ros::TransformListener> tf_listener_{nullptr};
    std::unique_ptr<tf2_ros::Buffer> tf_buffer_;

    // Simulate rotation with quaternions
    tf2::Quaternion last_orientation_;
    tf2::Quaternion orientation_increment_;

    // Used to reverse rotation direction after a while

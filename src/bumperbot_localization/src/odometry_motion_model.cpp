...
//Takes any angle in radians and wraps it into the range [-π, π].
double normalize(double z)
{
  return atan2(sin(z), cos(z));
}

//Computes the shortest signed angular difference between two angles a and b.
...
OdometryMotionModel::OdometryMotionModel(const std::string &name)
    ...
    , is_first_odom_(true)
{
    //Declare Parameters (allow override via config or launch)
    ...
    //Creates a vector of poses, initially all identical (default positions)
    if(nr_samples_ > 0){
    ...
    //Publishes updated particle cloud for visualization
...
{
    // Convert the robot's orientation from quaternion to roll, pitch, yaw
    ...
    // If this is the first odometry reading, initialize reference values and exit.
    if(is_first_odom_)
    {
	...
	// Compute difference (increment) from previous odometry
    double odom_x_increment = odom.pose.pose.position.x - last_odom_x_; //X1 - X
    double odom_y_increment = odom.pose.pose.position.y - last_odom_y_; //Y1 - Y
    double odom_theta_increment = angle_diff(yaw, last_odom_theta_);    //theta1 - theta
    ...
    for (auto & sample : samples_.poses)
    {
        ...
        // Get current orientation (yaw) of the particle
        ...
        // Update particle position using motion model with noise

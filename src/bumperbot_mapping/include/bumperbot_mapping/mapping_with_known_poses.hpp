...
//Log-odds form helps to combine multiple sensor readings efficiently
double prob2logodds(double p);
double logodds2prob(double l);

//Converts a pose in grid coordinates to an index in the occupancy grid array
unsigned int poseToCell(const Pose & pose, const nav_msgs::msg::MapMetaData & map_info);

//Converts world coordinates to grid coordinates (cells).
Pose coordinatesToPose(const double px, const double py, const nav_msgs::msg::MapMetaData & map_info);
    
//Checks whether the pose is inside the map boundaries.
bool poseOnMap(const Pose & pose, const nav_msgs::msg::MapMetaData & map_info);

//Returns the list of cells a laser beam passes through between the robot and a detected obstacle
std::vector<Pose> bresenham(const Pose & start, const Pose & end);

//estimates which cells are free or occupied based on a single laser measurement
